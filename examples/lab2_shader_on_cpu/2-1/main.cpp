...
int main()
{
    ...
    auto&& onchange = [](int val, void* rawData0) {
        ...
        auto&& mask = [&](vec2 uv) {
            vec3 raw = _rgb(sample<MLTSG_U8>(*fgImg, uv));
            vec3 bg = _rgb(sample<MLTSG_U8>(*bgImg, uv));
            return vec4(
        ...
        auto&& edgeBlur = [&](vec2 uv) {
            vec3 col(0.0f);
            float base = 0.0f;
            vec3 ct = _rgb(sample<MLTSG_U8>(*toBlur, uv, SampleUV::Clamp, SamplePoint::Point));
            ...
            for (float ddx = -size; ddx <= size + 0.1f; ddx += 1.0f)
            {
                for (float ddy = -size; ddy <= size + 0.1f; ddy += 1.0f)
                {
                    ...
                    vec3 smp = _rgb(sample<MLTSG_U8>(*toBlur, uv + offset / vec2(toBlur->cols, toBlur->rows),
        ...
        auto&& copy = [&](vec2 uv) {
            return sample<MLTSG_U8>(*toCopy, uv, SampleUV::Repeat, SamplePoint::Point);
        };

        toCopy = data0->image;
        multiProcess<MLTSG_U8, 32>(*data0->tmp0, copy);

        for (int i = 0; i < 4; ++i)
        {
            toBlur = i % 2 == 0 ? data0->tmp0 : data0->tmp1;
            multiProcess<MLTSG_U8, 32>(i % 2 == 1 ? *data0->tmp0 : *data0->tmp1, edgeBlur);
        }

        toCopy = data0->background;
        multiProcess<MLTSG_U8, 32>(*data0->tmp2, copy);

        for (int i = 0; i < 4; ++i)
        {
            toBlur = i % 2 == 0 ? data0->tmp2 : data0->tmp3;
            multiProcess<MLTSG_U8, 32>(i % 2 == 1 ? *data0->tmp2 : *data0->tmp3, edgeBlur);
        }

        bgImg = data0->tmp3;
        fgImg = data0->tmp1;
        multiProcess<MLTSG_U8, 32>(*data0->output, mask);

        bgImg = data0->background;
        fgImg = data0->image;
        multiProcess<MLTSG_U8, 32>(*data0->tmp0, mask);
