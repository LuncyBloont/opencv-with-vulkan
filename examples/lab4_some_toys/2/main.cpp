...
int main(int argc, char** args)
{
    ...
    {
        tmp.input = cv::imread("../images/dd.png");
    }
    ...
    multiProcess<MLTSG_U8, 16>(tmp.output, [&](glm::vec2 uv) { return texelFetch<MLTSG_U8>(tmp.input, uv); });

    cv::imshow("Drawer", tmp.output);

    multiProcess<MLTSG_U8, 16>(tmp.colorPan, [&](glm::vec2 uv) {
    ...
    auto update = [](int event, int x, int y, int flags, void* data) {
        ...
        if (tmp.bmdown)
        {
            switch (tmp.btype) {
                case BTYPE_ALPHA:
                    ...
                    break;
                case BTYPE_COLOR:
                    tmp.bcolor = texelFetch<MLTSG_U8>(tmp.colorPan, lmuv / glm::vec2(color_width, color_height));
                    break;
                case BTYPE_SIZE:
        ...
        multiProcess<MLTSG_U8, 16>(tmp.colorPan, [&](glm::vec2 uv) {
            ...
            return texelFetch<MLTSG_U8>(tmp.colorPan, uv);
        });
    ...
    cv::setMouseCallback("Drawer", [](int event, int x, int y, int flags, void* data) {
        ...
        if (tmp.pickup)
        {
            glm::vec4 color = texelFetch<MLTSG_U8>(tmp.output, muv / glm::vec2(tmp.output.cols, tmp.output.rows));
        ...
        if (tmp.ldown)
        {
            multiProcess<MLTSG_U8, 32>(tmp.output, [&](glm::vec2 uv) {
                ...
                {
                    return glm::mix(texelFetch<MLTSG_U8>(tmp.output, uv), tmp.color, tmp.color.a);
                }

                return texelFetch<MLTSG_U8>(tmp.output, uv);
            });
        }
